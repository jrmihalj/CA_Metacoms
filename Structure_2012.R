...
# Best model: 1covB (among others)

post.z <- NULL
post.z <- ggs(bundle_1covB, family="z")
...
z.post <- array(0, dim=c(Nsite_2012, Nspecies_2012, iter))
...
for(i in 1:iter){
  ...
  z.mat <- NULL
  z.mat <- matrix(subset, nrow=Nsite_2012, ncol=Nspecies_2012, byrow=F)
...
z.ord <- array(0, dim=c(nrow(z.post), Nspecies_2012, iter))
...
Structure2 <- replace(Structure2, which(Structure2=="Random"), "b_Rand")
...
Z_heat <- mat.or.vec(nr=nrow(z.post), nc=Nspecies_2012)
for(i in 1:iter){
...
colnames(Z_heat) <- as.factor(1:Nspecies_2012)
...
colnames(Z_heat) <- c("Alar", "Glob", "Echi", "Mano", "Rib")
...
caterplot(bundle_1covB, parms="p.detect", col="black", val.lim=c(0,1),
          labels=c("Alar", "Glob", "Echi", "Mano", "Rib"))


betalabs <- c(paste(c("Alar", "Glob", "Echi", "Mano", "Rib"),sep=""))
            
quartz(height=5, width=7)
caterplot(bundle_1covB, parms="betas", col="black", labels=betalabs)

quartz(height=4, width=6)
caterplot(bundle_1covB, parms="mean.beta.post", col="black",
          labels=c("Hydro"), las=0, val.lim=c(-2,0),
          horizontal=F)

quartz(height=4, width=6)
caterplot(bundle_1covB, parms="sd.beta.post", col="black",
          labels=c("Hydro"), las=0,
          horizontal=F)
