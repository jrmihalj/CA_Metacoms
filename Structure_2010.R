...
# Best model: 3cov (Lat + Snails_MDS2 + Snails_RA2)
...
z.post <- array(0, dim=c(Nsite_2010, Nspecies_2010, iter))
...
for(i in 1:iter){
  ...
  z.mat <- NULL
  z.mat <- matrix(subset, nrow=Nsite_2010, ncol=Nspecies_2010, byrow=F)
...
z.ord <- array(0, dim=c(nrow(z.post), Nspecies_2010, iter))
...
Z_heat <- mat.or.vec(nr=nrow(z.post), nc=Nspecies_2010)
for(i in 1:iter){
...
colnames(Z_heat) <- as.factor(1:Nspecies_2010)
